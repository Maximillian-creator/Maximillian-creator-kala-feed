"""
Themis-controle over de Kala-add-feed
=====================================
De teksten in de add-feed zijn LETTERLIJK die van kalahealth.nl. Wat daar mag
staan, mag bij Good For You niet automatisch ook: wij zijn zelf verantwoordelijk
voor elke claim op onze eigen pagina's (EFSA/NVWA/KOAG-KAG).
...
naast deze repo staat. In GitHub Actions is Themis er niet; dan stopt het script
netjes met een uitleg in plaats van een foutmelding. Daarom staat `published` in
de add-feed ook hard op false: bouwen mag, publiceren verdien je.
"""
...
def main():
    ...
    if themis is None:
        return 0
    if not ADD.exists():
        print("kala_add_feed.xml ontbreekt — draai eerst add_scraper.py")
        return 1
    ...
    kop = [
        "# Themis over de Kala Health add-feed",
        "",
        "De teksten hieronder komen letterlijk van kalahealth.nl. Alles wat hier",
        "staat moet aangepast zijn *voordat* een product in Shopify op",
        "'published' gaat.",
        "",
